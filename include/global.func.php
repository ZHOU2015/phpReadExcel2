...
function waplogin() {
	global $_userid, $MODULE, $DT_URL, $DT;
	$_userid or dheader("/member/waplogin.php");;
	//$_userid or dheader($MODULE[2]['linkurl'].$DT['file_login'].'?forward='.rawurlencode($DT_URL));
	return;
}

function wapoverpt($itemid) {
	global $_userid, $MODULE, $DT_URL, $DT;
	$_userid or dalert("积分不足!", "/gift1/show.php?itemid=".$itemid);
	return;
}
...
function send_sms($mobile, $message, $word = 0, $time = 0) {
	...
	$url = 'http://114.215.206.2/msg/HttpBatchSendSM'; //URL地址
	//群发地址 http://114.215.206.2/msg/HttpBatchSendSM
	
	if(!$DT['sms'] || !DT_CLOUD_UID || !DT_CLOUD_KEY || !is_mobile($mobile) || strlen($message) < 5) return false;
	$word or $word = word_count($message);
	
	$post_data = array();
	$post_data['account'] = DT_CLOUD_UID; //账号
	$post_data['pswd'] = DT_CLOUD_KEY; //密码
	$post_data['mobile'] = $mobile; //手机号码，使用逗号隔开可以发送多个号码
	$sms_message = convert($message, DT_CHARSET, 'UTF-8');

	// $message = '【神州软科】当前验证码：4413';
	// $message = iconv("gb2312", "utf-8//IGNORE", $message); //根据具体环境 设置对应字符集  接口响应UTF-8编码
	$post_data['msg'] = rawurlencode($sms_message); //短信内容
	...
	if ($arrret[1] == 0){
	   $code = $arrret[1];
		...
		$code = $err_info[$arrret[1]];
	    // echo '提交失败，错误代码：' . $arrret[1];
	}
...
function get_maincat($catid, $moduleid, $level = -1, $is_shop = 0, $is_sell = 0) {
	global $db;
	...
	$cat = array();
	$result = $db->query("SELECT catid,catname,child,style,linkurl,item,arrchildid FROM {$db->pre}category WHERE $condition ORDER BY listorder,catid ASC", 'CACHE');
	while($r = $db->fetch_array($result)) {
		$cat[] = $r;
	}
	return $cat;
...
function listpages($CAT, $total, $page = 1, $perpage = 20, $step = 2, $rt = '') {  // 生成分页，添加$rt 参数，控制生成链接，20160511 sunliya
	global $DT, $MOD, $L;
	if($total <= $perpage) return '';
	$items = $total;
	$total = ceil($total/$perpage);
	...
	$demo_url = $MOD['linkurl'].listurl($CAT, '{destoon_page}', $rt);  // 增加传参$rt
	$pages = '';
	include DT_ROOT.'/api/pages.'.((!$DT['pages_mode'] && $page < 100) ? 'default' : 'sample').'.php';
	return $pages;
}
...
function listurl($CAT, $page = 0, $rt = '') {
	global $DT, $MOD, $L;
	...
	if ($rt) $listurl = str_replace('list', $rt, $listurl);
	return $listurl;
}
