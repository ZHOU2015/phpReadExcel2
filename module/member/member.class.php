...
class member {
	...
	function is_member($member) {
		...
		//if($this->email_exists(trim($member['email']))) return $this->_($L['member_email_reg']); //去掉邮箱验证
		...
		if($this->userid) {
			...
			if($member['groupid'] > 5) {
				if(strlen($member['type']) < 1) return $this->_($L['member_type_null']);
	...
	function set_member($member) {
		...
		$member['mode'] = (isset($member['mode']) && $member['mode']) ? $member['mode'] : '';
		...
		//字母索引
		if(DT_CHARSET != 'UTF-8'){
			$zimu = convert($member['company'], 'UTF-8', DT_CHARSET);
		}else{
			$zimu = $member['company'];
		}
		$member['zimu'] = isset($member['zimu']) ? $member['zimu'] : strtoupper(substr(gb2py($zimu), 0, 1));
	...
	function add($member) {
		...
		$company_fields = array('username','groupid','company','type','catid','catids','areaid', 'mode','capital','regunit','size','regyear','sell','buy','business','telephone','fax','mail','address','postcode','homepage','introduce','thumb','keyword','linkurl','zimu');
	...
	function edit($member)	{
		...
		$company_fields = array('company','type','areaid', 'catid','catids','business','mode','regyear','regunit','capital','size','address','postcode','telephone','fax','mail','homepage','sell','buy','introduce','thumb','keyword','linkurl','groupid','domain','icp','validated','validator','validtime','skin','template','zimu');
	...
	function rename($cusername, $nusername) {
		...
		$tables = array('alert', 'ask', 'comment', 'finance_card', 'finance_cash', 'finance_charge', 'finance_pay', 'finance_deposit', 'finance_record', 'finance_sms', 'form_answer', 'form_record', 'guestbook', 'honor', 'link', 'admin_log', 'login', 'mail_list', 'spread', 'news', 'upgrade', 'news', 'page', 'address', 'oauth', 'vote_record', 'gift_order', 'poll_record', 'weixin_bind', 'weixin_user', 'member', 'member_check', 'company');
		foreach($MODULE as $m) {
			if($m['islink'] || $m['moduleid'] < 5) continue;
	...
	function check_get_shop() {
		$r = $this->db->get_one("SELECT content FROM {$this->db->pre}shop_check WHERE userid=$this->userid");
		return $r['content'] ? dstripslashes(unserialize($r['content'])) : array();
	}

	function check_add_shop($post) {
		global $DT_TIME;
		if(isset($post['content'])) {
			$content = dsafe($post['content']);
			unset($post['content']);
		...
		$content = addslashes(serialize($post));
		$this->db->query("REPLACE INTO {$this->db->pre}shop_check (userid,username,content,addtime) VALUES ('$this->userid','$this->username','$content','$DT_TIME')");
	}
